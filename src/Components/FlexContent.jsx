...
const FlexContent = ({ ifExists,endpoint:{title,heading,text,btn,url,img}}) => {
  return (
    <>
    <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ?'flex-row-reverse':'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full top-8'>
            <img src={img} alt={`img/${heading}`} className={`w-auto object-fill transitions-theme ${ifExists ?'h-60 lg:h-56  md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12':'h-72 lg:h-64 md:h-60 sm:h-48 rotate-[19deg] hover:rotate-12'}`}/>
        </div>
    </div>
    </>
  )
